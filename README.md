...
<img width="1320" height="651" alt="image" src="https://github.com/user-attachments/assets/adce54e2-418f-4efc-92d7-0cc865d02fec" />
...
Finday 백엔드 서버는 사용자의 금융 활동을 통합적으로 관리하며,  
금융결제원 Gateway 서버를 통해 각 은행 API와 연동하여 다양한 기능을 제공합니다.  
※ `🔁` 아이콘은 **금융결제원 서버를 통해 외부 은행 API와 연동**되는 기능입니다.
<br>
...
| 기능 | 설명 | 서버 위치 |
|------|------|------------|
| **회원가입** | 이메일, 비밀번호, 개인정보, 얼굴 이미지 입력 후 사용자 등록 | Finday 서버 |
| **로그인** | 이메일/비밀번호 + 얼굴 인식 기반 로그인 (JWT 발급) | Finday 서버 |
| **계좌 통합 조회** 🔁 | 금융결제원을 통해 연동된 은행 계좌 목록 및 잔액 조회 | 🔁 금융결제원 → 은행 |
| **거래 수동 생성** 🔁 | 사용자가 직접 거래 내용을 입력하고 거래 생성 요청 | 🔁 금융결제원 → 은행 |
| **거래 내역 조회** 🔁 | 계좌별 거래 내역을 조회 (기간 지정 가능) | 🔁 금융결제원 → 은행 |
| **계좌 이체** 🔁 | 출금 계좌 → 입금 계좌로 이체 요청, 얼굴 재인증 포함 | 🔁 금융결제원 → 은행 |
| **카드 조회 및 신청** 🔁 | 연동된 은행의 카드 목록 조회 및 신청 기능 제공 | 🔁 금융결제원 → 은행 |
| **소비 분석** | 거래 내역을 분석하여 카테고리별 소비 통계를 시각화 | Finday 서버 |
| **자산 종합 조회** 🔁 | 연동된 전체 계좌의 잔액을 합산하여 총 자산 계산 | 🔁 금융결제원 → 은행 |
